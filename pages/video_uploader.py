...
# # UPDATED QNA PROMPT: Strict Tech Stack Filter (No Topics/Concepts)
# CUSTOM_QNA_PROMPT = """
...
#    - 'Behavioral'
# 3. **Tech Stack (STRICT)**: 
#    - **Identify ONLY concrete Languages, Frameworks, Libraries, Databases, or Cloud Tools.**
#    - Examples: 'React.js', 'Node.js', 'Express.js', 'MongoDB', 'Python', 'Java', 'AWS', 'Docker'.
#    - **BANNED TOPICS/CONCEPTS**: DO NOT include concepts, protocols, or architecture terms.
#      - ❌ **REMOVE**: 'JWT', 'Sessions', 'Authentication', 'Authorization', 'Passport', 'API', 'REST', 'JSON', 'Frontend', 'Backend', 'Cookies', 'Middlewares'.
#    - **Example**: If question is "How do you handle JWT in React?", Tech Stack is **"React.js"** (Ignore JWT).
#    - **Example**: "Explain Database Indexing". Tech Stack is **"SQL"** or **"MongoDB"** (if context implies), otherwise "General Database".
#    - If no specific tool is mentioned, use "General" or "None".
...
def get_size_format(b, factor=1024, suffix="B"):
    """Scale bytes to its proper byte format (e.g., 1253656 => '1.20 MB')"""
...
app_dirs = [DOWNLOAD_DIR, TRANSCRIPT_DIR, QNA_DIR, CHUNKS_DIR]
total_app_usage = sum([get_directory_size(d) for d in app_dirs])
total_sys, used_sys, free_sys = shutil.disk_usage("/")

# SIDEBAR FOR SETTINGS & STORAGE
with st.sidebar:
    st.header("⚙️ Settings")
    st.info("Files are temporarily stored. Clear space if disk gets full.")
    ...
    col1.metric("App Files", get_size_format(total_app_usage))
    col2.metric("Free Space", get_size_format(free_sys))
    
    # Progress bar for system usage (Approximate visualization)
    usage_pct = min(used_sys / total_sys, 1.0)
    st.progress(usage_pct, text=f"Total System Disk Usage: {int(usage_pct*100)}%")
    
    if free_sys < 500 * 1024 * 1024: # Less than 500MB
        st.error("⚠️ Low Disk Space! Clear Storage.")

    st.divider()

    if st.button("🗑️ Reset & Clear Storage", type="primary"):
        n = clear_storage()
        st.cache_data.clear()
        st.success(f"Cleared {n} temporary files!")
        time.sleep(1)
